...
This document provides a complete blueprint for creating a new **TiTiler-pgSTAC** project with Azure OAuth authentication, based on the successful implementation in the rmhtitiler project.

**Key Success from rmhtitiler**:
...
13. [Migration from rmhtitiler](#migration-from-rmhtitiler)
...
### Knowledge from rmhtitiler

Review these documents from the rmhtitiler project:
...
- OAuth authentication from rmhtitiler
...
# OAuth function (copied from rmhtitiler custom_main.py)
def get_azure_storage_oauth_token() -> Optional[str]:
    """Get OAuth token for Azure Storage using Managed Identity."""
    # ... (exact copy from rmhtitiler custom_main.py:49-168)
    pass

# Middleware (adapted from rmhtitiler)
class AzureAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        # ... (adapted from rmhtitiler custom_main.py:171-194)
        pass
...
async def health():
    ...
    # ... (adapted from rmhtitiler custom_main.py:224-246)
...
async def startup_event():
    ...
    # Initialize OAuth (from rmhtitiler custom_main.py:276-318)
    if USE_AZURE_AUTH:
...
**Copy this function from rmhtitiler `custom_main.py:49-168`**:
...
**Copy this middleware from rmhtitiler `custom_main.py:171-194`**:
...
mkdir rmhtitiler-pgstac
cd rmhtitiler-pgstac

# Initialize git
git init

# Copy OAuth code from rmhtitiler
# You'll need custom_main.py from rmhtitiler as reference
```
...
# Create Web App
az webapp create \
  --name rmhtitiler-pgstac \
  --resource-group rmhazure_rg \
  --plan ASP-rmhtitiler-pgstac \
  --deployment-container-image-name rmhazureacr.azurecr.io/rmhtitiler-pgstac:1.0.0

# Configure ACR credentials
az webapp config container set \
  --name rmhtitiler-pgstac \
  --resource-group rmhazure_rg \
  --container-image-name rmhazureacr.azurecr.io/rmhtitiler-pgstac:1.0.0 \
...
az webapp identity show --name rmhtitiler-pgstac --resource-group rmhazure_rg
...
az postgres flexible-server ad-admin create \
  --server-name rmhpgstac \
  --resource-group rmhazure_rg \
  --display-name rmhtitiler-pgstac \
  --object-id $PRINCIPAL_ID

# Update DATABASE_URL to use Azure AD
# DATABASE_URL="postgresql://rmhtitiler-pgstac@rmhpgstac:@rmhpgstac.postgres.database.azure.com/pgstac?sslmode=require"
```
...
curl https://rmhtitiler-pgstac.azurewebsites.net/mosaic/searches | jq .
```

**Check tile coordinates**:
```bash
# Get correct bounds
curl "https://rmhtitiler-pgstac.azurewebsites.net/mosaic/{search_id}/info" | jq .bounds

# Use tilejson to find valid tiles
curl "https://rmhtitiler-pgstac.azurewebsites.net/mosaic/{search_id}/tilejson.json" | jq .
```
...
## Migration from rmhtitiler

### Key Differences

| Aspect | rmhtitiler | rmhtitiler-pgstac |
|--------|-----------|-------------------|
...
| **OAuth Code** | Identical | Identical (copy from rmhtitiler) |

### Code Reuse

**What to copy directly from rmhtitiler**:
...
1. **Create new project directory** (don't modify rmhtitiler)
2. **Copy OAuth authentication code** from rmhtitiler
...
### rmhtitiler Resources

- GitHub: https://github.com/rob634/rmhtitiler
- Deployment URL: https://rmhtitiler-ghcyd7g0bxdvc2hc.eastus-01.azurewebsites.net
...
def get_azure_storage_oauth_token() -> Optional[str]:
    ...
    [COPY ENTIRE FUNCTION FROM rmhtitiler custom_main.py:49-168]
    """
    # ... (complete implementation from rmhtitiler)
    pass
...
class AzureAuthMiddleware(BaseHTTPMiddleware):
    ...
    [COPY ENTIRE CLASS FROM rmhtitiler custom_main.py:171-194]
    """
    async def dispatch(self, request: Request, call_next):
        # ... (complete implementation from rmhtitiler)
        pass
...
This document provides everything needed to implement TiTiler-pgSTAC with OAuth authentication. Copy the OAuth code from rmhtitiler, adapt for pgSTAC, and follow the deployment steps. The OAuth approach is proven in production and will work identically for TiTiler-pgSTAC.

**Key Success Factor**: The OAuth authentication code is battle-tested and production-validated in rmhtitiler v2.0.0. Copy it exactly, adapt the endpoints for pgSTAC, and you'll have multi-container STAC catalog access working immediately.
