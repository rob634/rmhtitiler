...
# Login to ACR
az acr login --name geotileracr

# Get ACR credentials (for deployment)
az acr credential show --name geotileracr --query '{username:username,password:passwords[0].value}'
```
...
docker build \
  ...
  -t geotileracr.azurecr.io/titiler-azure:latest \
  -t geotileracr.azurecr.io/titiler-azure:v1.0.0 \
  -f Dockerfile \
  .
...
docker run -d \
  ...
  geotileracr.azurecr.io/titiler-azure:latest
...
```bash
# Push to ACR
docker push geotileracr.azurecr.io/titiler-azure:latest
docker push geotileracr.azurecr.io/titiler-azure:v1.0.0

# Verify image is in ACR
az acr repository list --name geotileracr --output table
az acr repository show-tags --name geotileracr --repository titiler-azure --output table
```
...
az webapp create \
  ...
  --deployment-container-image-name geotileracr.azurecr.io/titiler-azure:latest

# Configure ACR credentials
az webapp config container set \
  --resource-group rg-titiler-prod \
  --name rmh-titiler-prod \
  --docker-custom-image-name geotileracr.azurecr.io/titiler-azure:latest \
  --docker-registry-server-url https://geotileracr.azurecr.io \
  --docker-registry-server-user $(az acr credential show --name geotileracr --query username -o tsv) \
  --docker-registry-server-password $(az acr credential show --name geotileracr --query 'passwords[0].value' -o tsv)
```
...
- [ ] **Container Registry Name:** `geotileracr` or `_______` (must be globally unique)
