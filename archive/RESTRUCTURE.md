...
This document outlines a phased restructuring of the geotiler codebase from a single monolithic file into a well-organized Python package. The restructuring preserves all existing functionality while improving maintainability, testability, and developer experience.
...
|----------|-------------|
| Production (uvicorn) | `geotiler.app:create_app()` or `geotiler.app:app` |
| Docker | `uvicorn geotiler.app:app --host 0.0.0.0 --port 8000` |
| Development | `python -m geotiler` |
...
```bash
mkdir -p geotiler/{auth,routers,middleware,services,templates}
touch geotiler/__init__.py
touch geotiler/{auth,routers,middleware,services}/__init__.py
```

**Files to create:**
- `geotiler/__init__.py` - Version and package metadata
- `geotiler/config.py` - All configuration extraction
...
**Goal:** Extract all authentication logic into `geotiler/auth/`.
...
```python
# geotiler/auth/storage.py
import os
import logging
from datetime import datetime, timezone
from typing import Optional

from geotiler.config import settings, STORAGE_SCOPE
...
```python
# geotiler/auth/postgres.py
import logging
from datetime import datetime, timezone
from typing import Optional

from geotiler.config import settings, POSTGRES_SCOPE
...
```python
# geotiler/routers/health.py
from fastapi import APIRouter, Response
from datetime import datetime, timezone

from geotiler import __version__
from geotiler.config import settings, READYZ_MIN_TTL_SECS
from geotiler.auth.cache import storage_token_cache, postgres_token_cache, db_error_cache
from geotiler.services.database import ping_database
...
```python
# geotiler/routers/planetary_computer.py
...
**Create:** `geotiler/templates/pc_map.html`
...
from geotiler.routers import health, planetary_computer, root
...
Use `geotiler.app:app` instead.
"""
import warnings

warnings.warn(
    "custom_pgstac_main is deprecated. Use 'geotiler.app:app' instead.",
    DeprecationWarning,
    stacklevel=2
)

from geotiler.app import app
...
# After
CMD ["uvicorn", "geotiler.app:app", "--host", "0.0.0.0", "--port", "8000"]
```
...
from geotiler.auth.cache import TokenCache
...
- [ ] Phase 1: Foundation
  ...
  - [ ] Create `geotiler/__init__.py` with version
  - [ ] Create `geotiler/config.py` with Settings class
