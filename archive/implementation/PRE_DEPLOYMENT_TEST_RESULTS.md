...
### Prerequisites ✅
- [x] Managed Identity created (`geotileridentity`)
- [x] Managed Identity assigned to web app (`geotiler`)
...
PostgreSQL User: geotileridentity
...
```bash
curl "https://geotiler.azurewebsites.net/searches/{search_id}/tiles/WebMercatorQuad/14/11454/6143.png?assets=data" \
  -o tile.png
```
...
PGPASSWORD='B@lamb634@' psql \
  ...
  -U rob634 \
  -d geopgflex \
  -c "SELECT rolname FROM pg_roles WHERE rolname='geotileridentity';"
```
