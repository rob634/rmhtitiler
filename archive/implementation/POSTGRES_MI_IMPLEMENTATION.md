...
POSTGRES_USER=geotileridentity
POSTGRES_PORT=5432
...
The managed identity is already assigned to `geotiler` web app.

### 3. PostgreSQL User (Already Created) ✅

The PostgreSQL user `geotileridentity` has been created with:
- ✅ Login enabled
...
az webapp log tail --name geotiler --resource-group rmhazure_rg
...
```bash
# Health check
curl https://geotiler.azurewebsites.net/healthz | jq
...
curl -X POST "https://geotiler.azurewebsites.net/searches/register" \
  -H "Content-Type: application/json" \
  -d '{"collections":["system-rasters"],"limit":10}' | jq

# Test tile serving
curl "https://geotiler.azurewebsites.net/searches/{search_id}/tiles/WebMercatorQuad/14/11454/6143.png?assets=data" -o tile.png
```
...
Managed Identity returns OAuth token (valid ~1 hour)
    ↓
Build DATABASE_URL = postgresql://geotileridentity:{TOKEN}@rmhpgflex.postgres.database.azure.com/geopgflex
    ↓
...
```bash
az webapp identity show --name geotiler --resource-group rmhazure_rg
```
...
### Issue: "Role 'geotileridentity' does not exist"
...
PGPASSWORD='B@lamb634@' psql \
  ...
  -U rob634 \
  -d geopgflex \
  -c "SELECT rolname, rolcanlogin FROM pg_roles WHERE rolname = 'geotileridentity';"
```
...
PGPASSWORD='B@lamb634@' psql \
  ...
  -c "SELECT grantee, table_name, privilege_type
      ...
      WHERE grantee = 'geotileridentity'
